...
export const countryDeliveryAtom = atom<any>(undefined);

const CountryDeliveryField = ({ param, customOptions, field }: any) => {
  ...
  const setCountryDelivery = useSetAtom(countryDeliveryAtom);
  ...
  useEffect(() => {
    if (formData[field?.ColCodeSys]) {
      ...
      setCountryDelivery(formData[field?.ColCodeSys]);
    }
...
export default CountryDeliveryField;
