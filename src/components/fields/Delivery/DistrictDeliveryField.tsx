...
export const districtDeliveryAtom = atom<any>(undefined);

const DistrictDeliveryField = ({ param, customOptions, field }: any) => {
  const { component, formData } = param;

  const provinceDelivery = useAtomValue(provinceDeliveryAtom);
  const setDistrictDelivery = useSetAtom(districtDeliveryAtom);
  ...
  const { data }: any = useQuery(
    ["MstDistrictDelivery", provinceDelivery],
    async () => {
      if (provinceDelivery) {
        const resp: any = await api.Mst_District_GetByProvinceCode({
          ProvinceCode: provinceDelivery,
        });
  ...
  useEffect(() => {
    if (formData[field?.ColCodeSys]) {
      ...
      setDistrictDelivery(formData[field?.ColCodeSys]);
    } else {
      setValue(undefined);
...
export default DistrictDeliveryField;
