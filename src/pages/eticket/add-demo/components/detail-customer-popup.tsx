...
const DetailCustomerPopup = ({ code }: { code: string }) => {
  ...
  const {
    ...
    queryKey: ["getValueItem_Value", code],
    queryFn: async () => {
      if (code) {
        const response: any = await api.Mst_Customer_GetByCustomerCode([code]);
  ...
  const handleSubmit = () => {};
  return (
    <AdminContentLayout className={"province-management"} showSeparator={false}>
...
export default DetailCustomerPopup;
