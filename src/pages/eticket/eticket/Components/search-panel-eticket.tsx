...
import { useSavedStateEticket } from "./use-saved-state-eticket";
import { useSavedStateFormEticket } from "./use-saved-state-form-Eticket";
...
export const SearchPanelEticket = ({
  conditionFields = [],
  data,
  onSearch,
  storeKey,
  ...
  const { loadState, saveState } = useSavedStateEticket<ColumnOptions[]>({
    storeKey: `search-panel-settings-${storeKey}`,
  });
  const { saveStateForm } = useSavedStateFormEticket<any>({
    storeKey: `search-panel-settings-${storeKey}`,
  });
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      const outputColumns = savedState.map((column: ColumnOptions) => {
        const filterResult = conditionFields.filter((c: ColumnOptions) => {
          return c.dataField === column?.dataField && c.visible;
        });
