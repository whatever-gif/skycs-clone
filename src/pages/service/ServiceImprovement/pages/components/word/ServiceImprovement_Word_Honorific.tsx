...
import { Lst_SvImp_SvImprvHonorificAtom } from "../../ServiceImprovementPage";
import { DeleteButton } from "./ServiceImprovement_Word_Deny";
...
const ServiceImprovement_Word_Honorific = forwardRef(
  ({ readOnly }: any, ref: any) => {
    ...
    const [listData, setListData] = useAtom(Lst_SvImp_SvImprvHonorificAtom);
    ...
    return (
      <div className="w-[50%] pb-[200px]">
        <div className="flex justify-between">
          <div className="font-semibold">{t("Lst_SvImp_SvImprvHonorific")}</div>
          {readOnly ? (
        ...
        <div className="flex flex-col gap-4 py-2 pb-4 px-2 w-full border-[1px] mt-1">
          <div className="flex items-center gap-2">
            ...
            <div className="font-semibold">{t("QtyStdHonorific")}</div>
            <div className="font-semibold w-[150px]">{t("FlagIsRequire")}</div>
          </div>
              ...
              <div className="flex items-center gap-2" key={item?.Idx}>
                ...
                >
                  ...
                  {(!item.WordDesc || item?.length <= 0) && (
                    <div className="absolute text-[11px] text-red-500">
                      {validateMsg("WordDescHonorific is required!")}
                    </div>
                  )}
                </div>
                ...
                <div className={`relative border-[1px] rounded-[5px] `}>
                  ...
                  {(!item.QtyStd || item.QtyStd <= 0) && (
                    <div className="absolute text-[11px] text-red-500">
                      {validateMsg("QtyStdHonorific is required!")}
                    </div>
                  )}
                </div>

                <SelectBox
                  dataSource={listFlagIsRequire}
                  displayExpr="label"
                  valueExpr="value"
                  width={150}
                  onValueChanged={(e: any) =>
                  ...
                  defaultValue={item?.FlagIsRequire}
                  readOnly={readOnly}
                />
              </div>
            );
...
export default ServiceImprovement_Word_Honorific;
