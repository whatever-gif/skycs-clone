...
const RptCallAnalysisByCallTime = () => {
  const { t } = useI18n("RptCallAnalysisByCallTime");
  ...
  const { data: getData, isLoading }: any = useQuery(
    ["getCallTime", currentValue, currentCode],
    async () => {
      ...
      const resp: any = await api.RptCallAnalysis_RptCallAnalysisByCallTime({
  ...
  const tableDetail = useMemo(() => {
    ...
    return (
      <GridViewPopup
        ...
        storeKey={"RptCallAnalysisByCallTime-search"}
        editable={false}
        checkboxMode="none"
        height={500}
        toolbarItems={[
  ...
  const handleSave = async () => {
    ...
    const Rpt_CallAnalysis = {
      ...
      RptCallAnalysisType: "RPTCALLANALYSISBYCALLTIME",
  ...
  return (
    <AdminContentLayout
      className={"RptCallAnalysisByCallTime h-full w-full rpt-container"}
    >
      <AdminContentLayout.Slot name={"Header"}>
        <div className="header flex justify-between items-center w-full px-2">
          <div className="font-semibold">{t("RptCallAnalysisByCallTime")}</div>
          <div className="flex">
            <PermissionContainer
              permission={"BTN_RPT_PHAN_TICH_THOI_LUONG_CUOC_GOI_EXPORT_EXCEL"}
            >
              <Button
                style={{
                  padding: 10,
            ...
            <PermissionContainer
              permission={"BTN_RPT_PHAN_TICH_THOI_LUONG_CUOC_GOI_LUU_RPT"}
            >
              <Button
                style={{
                  padding: 10,
      ...
      <AdminContentLayout.Slot name={"Content"}>
        <ContentSearchPanelLayout searchPermissionCode="">
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <div className="w-full h-full d-flex-col pb-2">
              ...
              {!currentValue.DataSource || !currentCode.SvImprvCodeSys ? (
                ...
                <>
                  ...
                  <GridViewCustomize
                    nameFile={
                      RP("Phân tích thời lượng cuộc gọi") +
            ...
            <Popup
              visible={openDetail}
              title={`${t("RptCallAnalysisByCallTime Detail")}`}
              showCloseButton
              hideOnOutsideClick
...
export default RptCallAnalysisByCallTime;
