...
const defaultName = `Báo cáo Phân tích tần số âm thanh cuộc gọi ${format(
  new Date(),
  "yyyy-MM-dd"
).toString()}`;
...
const SaveNamePopup = forwardRef(
  ({ openPopup, handleClosePopup, handleSave }: any, ref: any) => {
    const { t } = useI18n("RptAnalysisBySoundValueFreq");
    ...
    return (
      <Popup
        visible={openPopup}
        title={`${t("RptCallAnalysisBySoundValueFreq Save")}`}
        showCloseButton
