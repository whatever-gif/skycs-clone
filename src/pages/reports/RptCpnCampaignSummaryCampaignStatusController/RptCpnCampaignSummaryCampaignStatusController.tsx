...
import { RptCpnCampaignSummaryCampaignStatusData } from "@/packages/types";
...
const RptCpnCampaignSummaryCampaignStatusController = () => {
  const { t } = useI18n("Rpt_CpnCampaignSummaryCampaignStatusController");
  ...
  const {
    ...
    queryFn: async () => {
      if (key !== "0") {
        const response = await api.RptCpnCampaignSummaryCampaignStatus_Search(
          searchCondition
        );
        if (response.isSuccess) {
          // console.log("response ", response);
          const data = response.Data
            ? response.Data.Rpt_Cpn_CampaignSummaryCampaignStatus ?? []
            : [];

          const customize = data.map((item: any) => {
            return {
              ...item,
              ValRateCCStatus: parseFloat(item.ValRateCCStatus),
            };
          });
          return customize;
        } else {
    ...
    <AdminContentLayout className={"Category_Manager"}>
      <AdminContentLayout.Slot name={"Header"}>
        <p className="header-title p-2">
          {t("Rpt_CpnCampaignSummaryCampaignStatusController")}
        </p>
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <SearchPanelV2
              ...
              storeKey="RptCpnCampaignSummaryCampaignStatusController"
              data={searchCondition}
              onSearch={handleSearch}
              permission="MNU_RPT_CPNCAMPAIGN_SUMMARY_STATUS_CONTROLLER_SEARCH"
            />
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <div className="flex align-items-start justify-content-center h-full py-4">
              {key !== "0" && !isLoadingValue && (
                <div className="flex align-items-center">
                  ...
                  >
                    <PieSeries
                      argumentField="CampaignCustomerStatus"
                      valueField="ValRateCCStatus"
                    >
                      <Label
                        visible={true}
                    ...
                    {data.map(
                      (item: RptCpnCampaignSummaryCampaignStatusData) => (
                        <Annotation
                          key={item.QtyCCStatus}
                          data={item.ValRateCCStatus}
                        ></Annotation>
                      )
                    )}
                  </PieChart>

                  <div className="flex justify-center transform-state">
                    <div className="table-pivot-content">
                      <table className="pivot-table">
                        <tr className="header">
                          <th className="th-header">
                            {t("State of Campaign Perform")}
                          </th>
                          ...
                          <th className="td-content text-right">
                            {data.reduce(
                              (
                                ...
                                return total + item.ValRateCCStatus;
                              },
                              0
                            )}
                          </th>
            ...
            <div className="RptCpnCampaignSummaryCampaignStatusController-manager"></div>
          </ContentSearchPanelLayout.Slot>
        </ContentSearchPanelLayout>
      </AdminContentLayout.Slot>
    </AdminContentLayout>
...
export default RptCpnCampaignSummaryCampaignStatusController;
